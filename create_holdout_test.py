"""
Create a proper held-out test set for evaluation

This script creates new test questions that are NOT in the training data
to avoid data leakage and get honest evaluation results.
"""
...
def create_holdout_test_set():
    """Create test questions that are completely separate from training data"""
    
    # These are NEW questions not in the training set
    holdout_questions = [
        {
    ...
    holdout_file = output_dir / "holdout_test_set.json"
    with open(holdout_file, 'w', encoding='utf-8') as f:
        json.dump(holdout_questions, f, indent=2, ensure_ascii=False)
    
    print(f"✅ Created holdout test set: {holdout_file}")
    print(f"📊 Test questions: {len(holdout_questions)}")
    print(f"📋 Categories: {set(q['category'] for q in holdout_questions)}")
    
    # Show sample
    print(f"\n📝 Sample holdout question:")
    sample = holdout_questions[0]
